...
vlog <- variogram(log(distance_from_home) ~ distance, data = data_spatial)
plot(vlog)
# non converge, dato che c'? poca variabilit? tra i dati
vlog <- variogram(log(distance_from_home) ~ distance, data = data_spatial,boundaries = c(0,200,seq(400,6000,450)))
plot(vlog)
...
vlog.fit
#with box-cox
vbc <- variogram(bc.distance_from_home ~ distance, data = data_spatial,boundaries = c(0,200,seq(400,6000,450)))
plot(vbc)
vbc.fit <- fit.variogram(vbc, vgm(0.3, "Exp", 3000, 0.05)) #o convergence after 200 iterations: try different initial values?
x11()
...
#--------------------------------
# non stationary variogram v
vlog <- variogram(log(dist_from_home) ~ dist, data = data_spat)
plot(vlog)
# non converge, dato che c'? poca variabilit? tra i dati
vlog <- variogram(log(dist_from_home) ~ dist, data = data_spat,boundaries = c(0,200,seq(400,6000,450)))
plot(vlog)
vlog.fit <- fit.variogram(vlog, vgm(0.3, "Exp", 3000, 0.05))
x11()
plot(vlog, vlog.fit, pch = 3)
vlog.fit
#with box-cox
vbc <- variogram(bc.dist_from_home ~ dist, data = data_spat,boundaries = c(0,200,seq(400,6000,450)))
plot(vbc)
vbc.fit <- fit.variogram(vbc, vgm(5000, "Exp", 3000, 1000))
x11()
plot(vbc, vbc.fit, pch = 3)
vlog.fit
#non convergono in fondo a un asintoto cmq
#--------------------------------
...
# non stationary variogram v
vlog <- variogram(log(distance_from_home) ~ DUMMY + distance + distance*DUMMY, data = data)
plot(vlog)
# non converge, dato che c'? poca variabilit? tra i dati
vlog <- variogram(log(distance_from_home) ~ DUMMY + distance + distance*DUMMY, data = data,boundaries = c(0,200,seq(400,6000,450)))
plot(vlog)
vlog.fit <- fit.variogram(vlog, vgm(0.3, "Exp", 3000, 0.05))
x11()
plot(vlog, vlog.fit, pch = 3)
vlog.fit
#with box-cox
vbc <- variogram(bc.distance_from_home ~ DUMMY + distance + distance*DUMMY, data = data_spatial,boundaries = c(0,200,seq(400,6000,450)))
plot(vbc)
