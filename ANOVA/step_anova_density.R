...
for(i in 1: n){
  ...
  dev[(i*7 - 6): (i*7)] <- c(mean(lun)/area_cbg[j], mean(mar)/area_cbg[j], 
...
fit <- aov(dev ~ days)
summary(fit)
#QUA VIENE PAVLUE ALTOOOOOOOO E' CAMBIATO DA PRIMA CHE VENIVA BASSO
#0.534 fuck
...
fit <- aov(dev ~ days)
summary(fit)
#MI SEMBRA PVALUE ALTO 0.169 -> NO EVIDENCE TO REFUSE H0 
#(assunzioni no buone, però anche qua visivamente non ce lo aspettavamo)
...
par(mfrow=c(2,1))
barplot(rep(Mdays,each=b), names.arg=c('','','mon', '','','','','tue', '','','','','wed', '','','','','thu', '','','','','fri', '','','','','sat', '','','','','sun', '','')
,
...
x11()
par(mfrow=c(2,1))
barplot(c(rep(Mcounty,times=g-4), rep(0, times = 4*5)), names.arg=c('','','mon', '','','','','tue', '','','','','wed', '','','','','thu', '','','','','fri', '','','','','sat', '','','','','sun', '','')
,
...
, 
        col = rep(colcountyNY[1:5], times = g),main='Additivie model Days+County NY') 
#qua coloro allo stesso modo i giorni, bordo indicica la county
...
,
        col=rep(colcountyNY[1:5], times= g), main='Model with Interact. Days+County NY')
dev.off()
x11()
boxplot(dev ~ days)
...
for (l in 1: length(list_county)){ #loop on counties
  ...
  #x11()
  png(file =paste(list_county[l],'barplot anova_days.png', sep = ""))
  par(mfrow=c(1,2), las =2)
  barplot(rep(mean(dev),7), names.arg=levels(days),
          col='grey85', main=paste('Model under H0', list_county[l])) # ylim=c(0,max(dev))
  barplot(tapply(dev, days, mean), names.arg=levels(days),
          col=coldays,main=paste('Model under H1', list_county[l])) #, ylim=c(0,max(dev))
